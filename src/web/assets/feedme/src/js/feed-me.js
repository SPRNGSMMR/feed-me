...
$(function () {
  ...
  $(document).on('change', '#elementType', function () {
    ...
    if (value === 'craft-elements-User') {
      $disableLabel.text(Craft.tt('feed-me-rqen', 'Suspend missing users'));
      $disableInstructions.text(
        Craft.tt('feed-me-rqen', 'Suspends any users that are missing from the feed.')
      );
    } else {
  ...
  $(document).on('change', '.element-parent-group select', function () {
    ...
    var newOptions =
      '<option value="">' + Craft.tt('feed-me-rqen', 'None') + '</option>';
