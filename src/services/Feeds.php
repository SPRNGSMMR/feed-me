...
class Feeds extends Component
{
    ...
    public function saveFeed(FeedModel $model, bool $runValidation = true): bool
    {
        ...
        if ($isNewModel) {
            ...
            if (!$record) {
                throw new Exception(Craft::t('feed-me-rqen', 'No feed exists with the ID “{id}”', ['id' => $model->id]));
            }
        }
    ...
    public function getModelOverrides($handle, $feedId): mixed
    {
        if (empty($this->_overrides[$feedId])) {
            $this->_overrides[$feedId] = Hash::get(Craft::$app->getConfig()->getConfigFromFile('feed-me-rqen'), 'feedOptions.' . $feedId);
        }
    ...
    private function _getFeedRecordById(int $feedId = null): FeedRecord
    {
        if ($feedId !== null) {
            ...
            if (!$feedRecord) {
                throw new FeedException(Craft::t('feed-me-rqen', 'No feed exists with the ID “{id}”.', ['id' => $feedId]));
            }
        } else {
